...
export const CMSModal: React.FC<CMSModalProps> = ({
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    try {
      return (
        localStorage.getItem('sheida_admin_auth') === 'true' ||
        sessionStorage.getItem('sheida_admin_auth') === 'true' ||
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    // Valid credentials:
    // Usernames: 'admin', 'sheida', 'sheidahair', 'admin@sheidahair.ca', 'manager'
    // Passwords: 'sheida2026', 'sheida124', '124', 'admin', 'themaze2026'
    const validUsernames = ['admin', 'sheida', 'sheidahair', 'admin@sheidahair.ca', 'manager', 'admin@themaze.ca'];
    const validPasswords = ['sheida2026', 'sheida124', '124', 'admin', 'themaze2026'];
    ...
    setTimeout(() => {
      ...
      if (validUsernames.includes(user) && validPasswords.includes(pass)) {
        ...
        try {
          if (rememberMe) {
            localStorage.setItem('sheida_admin_auth', 'true');
          } else {
            sessionStorage.setItem('sheida_admin_auth', 'true');
          }
        } catch {
          // ignore
        }
  ...
  const handleAdminLogout = () => {
    ...
    try {
      localStorage.removeItem('sheida_admin_auth');
      sessionStorage.removeItem('sheida_admin_auth');
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-xs font-bold tracking-widest text-white uppercase">
                  Sheida Hair Studio • Admin Portal
                </h2>
          ...
          <form onSubmit={handleAdminLogin} className="p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. admin or sheida"
                required
            ...
            <div className="p-3 bg-white/5 border border-white/5 rounded text-[11px] text-zinc-400 font-mono space-y-1">
              <p className="text-zinc-300 font-bold text-[10px] uppercase tracking-wider">Admin Login Credentials:</p>
              <p>👤 <span className="text-zinc-200">Username:</span> <code className="text-[#d8b485]">admin</code> or <code className="text-[#d8b485]">sheida</code></p>
              <p>🔑 <span className="text-zinc-200">Password:</span> <code className="text-[#d8b485]">sheida2026</code></p>
            </div>
          </form>
        </div>
      ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-500">
                  124 Willowdale Ave • Manage bookings, portfolio transformations and pricing
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6 text-sm text-zinc-300">
              ...
              <div className="p-6 bg-red-950/20 border border-red-500/20 space-y-4">
                <h4 className="text-xs font-bold text-red-300 uppercase tracking-wider">
                  Reset Studio Content
                </h4>
                <p className="text-xs text-zinc-400 leading-relaxed">
                  Restore the initial Sheida Hair Studio portfolio transformations, prices, and services from the North York studio.
                </p>
                <button
                  onClick={() => {
                    if (confirm('Reset portfolio and services to initial Sheida Hair Studio data?')) {
                      onResetDefaults();
