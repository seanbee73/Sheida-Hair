...
export const BookingRequestsManager: React.FC<BookingRequestsManagerProps> = ({
  ...
  const handleCreateManual = (e: React.FormEvent) => {
    ...
    const created: BookingInquiry = {
      ...
      preferredStylist: newBooking.preferredStylist || 'Sheida (Founder & Master Colorist)',
      serviceCategory: newBooking.serviceCategory || 'Signature Dimensional Balayage',
  ...
  const handleCopyConfirmationText = (b: BookingInquiry) => {
    const text = `Hi ${b.name}! This is ${SALON_INFO.name} (${SALON_INFO.phone}) confirming your booking request for ${b.serviceCategory} with ${b.preferredStylist} on ${b.preferredDate || 'your requested date'} at ${b.preferredTime}. We look forward to welcoming you at 124 Willowdale Ave, North York!`;
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `sheida_hair_studio_bookings_${new Date().toISOString().split('T')[0]}.csv`);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="sm:col-span-2">
              ...
              <select
                ...
                className="w-full bg-[#111114] border border-white/10 px-3 py-2 text-xs text-white rounded focus:border-[#d8b485] focus:outline-none"
              >
                <option value="Haircut & Blowdry with Haik">Haircut &amp; Blowdry with Haik ($120+)</option>
                <option value="Signature Dimensional Balayage">Signature Dimensional Balayage ($400+)</option>
                <option value="Full Highlights & Babylights">Full Highlights &amp; Babylights ($500+)</option>
                <option value="Bleach & Full Blonding Transformation">Bleach &amp; Full Blonding Transformation ($500+)</option>
                <option value="Root Touch-Up & Gray Coverage">Root Touch-Up &amp; Gray Coverage ($100+)</option>
                <option value="Milbon 5-Step Japanese Moisture Treatment">Milbon 5-Step Japanese Moisture ($100)</option>
                <option value="Keratin Smoothing Treatment">Keratin Smoothing Treatment ($400+)</option>
                <option value="Hair Botox & Protein Rebuild">Hair Botox &amp; Protein Rebuild ($400+)</option>
                <option value="Haircolor Consultation + Strand Test">Haircolor Consultation + Strand Test ($0)</option>
              </select>
            </div>
